...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top">
      <div class="container">
        <a class="navbar-brand" href="index.php">Bikes &amp Cars</a>
...
if(isset($_POST['log'])){
  $unm=$_POST['unm'];
  $psw=$_POST['psw'];
  $q=$mysqli->query("select * from puser where unm='$unm' and psw='$psw'") or die("unable to select");
  $c=$q->num_rows;
  if($c>0){
  /*if(isset($_GET['a'])){
    if($_GET['a']=='bk'){
...
if(isset($_POST['reg'])){
  ...
  $unm=$_POST['punm'];
  $psw=$_POST['ppwd'];
  $q=$mysqli->query("insert into puser (`name`,`unm`,`psw`) values('$name','$unm','$psw')");
  if($q==1){
